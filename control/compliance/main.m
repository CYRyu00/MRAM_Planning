...
%TODO : check
tau_quad = 0; tau_servo = 0; tau_disturb = 0; phi_i = zeros(1, n_am); 
[pdd_i, phidd_tool, phidd_quad] = ForwardDynamics(force, tau_quad, tau_servo, tau_disturb, ...
        p_i, phi_i, J_tool(2,2), J_quad(2,2), l1, l2, R_c, n_am, mass, g, k_s, pc_i)
...
%% TODO
function [pdd_i, phidd_tool, phidd_quad] = ForwardDynamics(force, tau_quad, tau_servo, tau_disturb, p_i, phi_i, J_tool, J_quad, l1, l2, R_c, n_am, mass, g, k_s, pc_i)
    e_1 = [1;0;0]; e_2 = [0;1;0]; e_3 = [0;0;1];
    ...
    for i = 1:n_am
        ...
        if i > 1
            q(:, i) = p_i_tilde(:, i) - p_i_tilde(:, i-1) + l1*(Ry(phi_i(i)) - R_c) *e_1 + l2*(Ry(phi_i(i-1)) - R_c) *e_1;
        end
    end

    pdd_i = zeros(3, n_am);
    for i = 1:n_am
        pdd_i(:, i) = -g * [0;0;1] + (force(:, i) - k_s(i) * q(:, i) + k_s(i+1) * q(:, i+1)) / mass(i);
    end
    
    phidd_tool = zeros(1, n_am);
    phidd_quad = zeros(1, n_am);
    for i = 1:n_am
        
        term = l1 * S(e_1) * (- k_s(i) * q(:, i)) + l2 * S(e_1) * (k_s(i+1) * q(:, i+1));
        phidd_tool(i) = J_tool\(tau_servo + term(2) + tau_disturb);
        phidd_quad(i) = J_quad\(tau_quad - tau_servo);
    end
end
