...
for eta = eta_arr
    ...
    for i = 1:size(thrusts, 1)
        u = thrusts(i, :)';
        f = A_force * u;
        if norm(f) < 1e-6  % force ≈ 0
            tau = A_tau * u;
            valid_taus = [valid_taus; tau'];
        end
    end
    % Convex hull of valid taus
    if isempty(valid_taus)
        disp('No valid torque found that satisfies force = 0');
    else
        K = convhull(valid_taus(:,1), valid_taus(:,2), valid_taus(:,3));
        ...
        axis equal
        trisurf(K, valid_taus(:,1), valid_taus(:,2), valid_taus(:,3), ...
            'FaceAlpha', 0.6, 'EdgeColor', 'k');
        xlabel('$\tau_x$', 'Interpreter', 'latex');
        ylabel('$\tau_y$', 'Interpreter', 'latex');
        zlabel('$\tau_z$', 'Interpreter', 'latex');
        
        title_eta = sprintf('Torque Polytope force = 0, $\\eta = %.2f^\\circ$', eta * 180 / pi);
        title(title_eta, 'Interpreter', 'latex');

        grid on;
        axis equal;
        xlim([-0.3 0.3]); ylim([-0.3 0.3]); zlim([-0.5 0.5]);  % 원하는 크기로 조정
    end
end
%%
function out = Ad(R, p)
