...
options_qp = optimoptions('quadprog', ...
...
% wrech polytop weight
Q = diag([1.75 1.0 1.0]);% ty, fx, fz, 클수록 덜 중요
Q = Q / det(Q)^(1/3);
...
theta_tilde = 30 / 180 *pi;

center_axis = mean;
angle_deg = 20;
num_points = 500;
bound_ratio = 0.7;
U = sample_3d_cone_process(center_axis, angle_deg, num_points, bound_ratio);
U = Q * U;
...
while  ~does_success || iter <=n_iter
    ...
    lb = max(theta_min, theta - theta_tilde * ones(num_AMs, 1));
    ub = min(theta_max, theta + theta_tilde * ones(num_AMs, 1));
    
    [theta_opt_cand, opt_f_cand, exitflag, output, ~, ~, ~] = ...
    fmincon( @(theta_)high_cost_sparse(theta_, beta, U, num_AMs, num_points, thrust_lb, thrust_ub, R_shape, r_cj, B_tau, B_lambda), ...
             theta_init, [], [], [], [], lb, ub, [], options_fmincon);

    if (exitflag > 0) && opt_f_cand < opt_f 
...
ewma_hist = []; ewmv_hist = cell(N_sim, 1); wrench_des_hist = []; center_axis_hist = []; angle_deg_hist = []; opt_f_hist = [];
...
for i = 1:N_sim
    ...
    if mod(i, step_config_opt) == 1 || step_config_opt == 1
        if i == 1
            ...
            num_points = 500;
            bound_ratio = 0.7;
            U = sample_3d_cone_process(center_axis, angle_deg, num_points, bound_ratio); % TODO sample at only first
            U = Q * U; 
            beta = 1e-1;
            % theta_min = -60 / 180 *pi * ones(num_AMs, 1);
            % theta_max = 60 / 180 *pi * ones(num_AMs, 1);
            ...
            n_iter = 3; opt_f = 0.0; does_success = false; iter = 1;
            tic
            while  ~does_success || iter <=n_iter
                theta_init = theta + (2*rand(num_AMs, 1) - 1) * 5 / 180 * pi; 
                lb = max(theta_min, theta - theta_tilde * ones(num_AMs, 1));
    ...
    if mod(i, step_central_control) == 1 || step_central_control == 1
        % MBO
        h = [m_c * Xd; e_2' * J_tool_tot * w + It(2,2) * sum(phid_q)]; % fx fy fz, tauy
        ...
        [sol, fval, exitflag, output] = quadprog(H_qp, f_qp, A_ineq, b_ineq, A_eq, b_eq, lb_qp, ub_qp, sol_init, options_qp);
    ...
    center_axis_hist = [center_axis_hist, center_axis]; 
    angle_deg_hist = [angle_deg_hist, angle_deg];
...
legend({'$f_x$','$f_y$','$f_z$', '$\tau_y$'},'Interpreter','latex','FontSize', 12);
...
for i = 1:dN:N_sim
    ...
    % cone  
    center = [X_hist(1, i), X_hist(3, i)];
    radius = -opt_f_hist(i) * 0.003;
    alpha = atan2(center_axis_hist(3, i), center_axis_hist(2, i));
    t_ = linspace(alpha - angle_deg_hist(i)/ 180 *pi, alpha + angle_deg_hist(i)/ 180 *pi, 100);
    x_ = [center(1), center(1) + radius * cos(t_), center(1)];
    y_ = [center(2), center(2) + radius * sin(t_), center(2)];
    
    % 4. 시각화 (fill 함수 사용)
    fill(x_, y_, 'c', ...                % 'c'는 기본 FaceColor (하늘색)
        'FaceColor', [1.0 0.7 0.7], ...  % RGB 값으로 정밀한 내부 색상 설정 (연한 파랑)
        'EdgeColor', [1.0 0.5 0.5], ...  % 테두리 색상 (진한 파랑)
        'LineWidth', 1, ...            % 테두리 두께
        'FaceAlpha', 0.6);             % 내부 투명도 (0: 투명, 1: 불투명)
