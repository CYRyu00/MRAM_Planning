...
k_mbo = diag([1.0 1.0 1.0 1.0]) * 6e0; % 3e0
dt_lpf = 0.2;

% optimization
tan_max = tan(60 / 180 * pi);
...
for i = 1:N_sim
    ...
    if mod(i, delay_mbo) == 1 || delay_mbo == 1
        ...
        b_eq = [f_ext_hat(1:3); e_2 * f_ext_hat(4) - S(r_e) * f_ext_hat(1:3)]...
         - A_eq * [delta_n_interp(:, i); 0 ; 0]; % w.r.t ee 
        ...
        for j = 1:num_AMs
            ...
            b_soc = v(3*j-2:3*j) - delta_n_interp(3*j-2:3*j, i);
        ...
        A_ineq = [A1, zeros(num_AMs, 2);...
                  A2, zeros(num_AMs, 2)];
        b_ineq = [A1 * (v - delta_n_interp(:, i)); A2 * (v - delta_n_interp(:, i))];
        ...
        F_int0 = A_dagger *([(v - delta_n_interp(:, i)); zeros(3*num_AMs, 1)] + F_ext - Cqd) + ((A* (M\ A')) \ Ad) * qd; % mge_3 doesn't generate internal forces;
        ...
        if exitflag == 1
            ...
            delta_reshape = reshape(sol(1:3*num_AMs) + delta_n_interp(:, i), 3, num_AMs);
        ...
        internal_wrench = A_delta * (delta_prev + delta_n_interp(:, i)) + F_int0;
    ...
    Xdd = (M_o + AM_mass * mass_uncertainty)\(force_tot - (M_o + AM_mass * mass_uncertainty) * 9.81 * e_3 + disturb(1:3) + f_fric * e_1);
    Xdd(2) = 0; Xdd(3) = 0;
    % if abs(Xd(1)) < 1e-3
...
%% force/disturbance
figure('Position',[1350 100 400 900]);
...
min_X = min([X_des(:, 1:N_sim), X_hist], [], 2); % Minimum for each row (x, y, z)
max_X = max([X_des(:, 1:N_sim), X_hist], [], 2); % Maximum for each row (x, y, z)
