...
force_prev = ones(num_AMs) * m0 * 1.0;
force = ones(num_AMs) * m0 * 1.0;
...
for i = 1:N_sim_tmp
    ...
    for j = 1:num_AMs
        ...
        nu_ej = force_prev(j) * R *e_3 - mj * Xdd_des(:, i) + kv_j * e_pd + kp_j * e_p + mj * gravity;
        eta = - alpha * nu_ej ...
              - mj * gamma * (e_pd + epsilon * e_p);
        vec = R' * (eta + mj * Xddd_des + kv_j * e_pdd_hat + kp_j * e_pd);
...
ylim([-5* thrust_limit, 5* thrust_limit] )
...
hold on
for j = 1:3
    mask = ~isoutlier(nu_e_hist(j, :));
    plot(times(mask), nu_e_hist(j, mask), 'Color', colors(j,:), 'LineWidth', 1.0);
end
legend({'$x$','$y$','$z$'},'Interpreter','latex','FontSize', 12);
title('$\nu_e$', 'Interpreter', 'latex','FontSize', 14)
grid on