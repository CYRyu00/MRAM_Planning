...
box_size = { [1, 0.02, 2], [0.15, 0.05, 0.03],[0.04, 0.04, 0.1],[2 * d * 0.9, 0.04, d * 0.9] ,[0.01,0.01,0.01]} ; 
...
for i = 1:4
    ...
    if i == 4
        ...
        for j =1:length(AMs_cols)
            r = [ (core(2) - AMs_cols(j)) * -2 * d ; (core(1) - AMs_rows(j)) *d ;0];% p_j,core
...
%close all
%figure('Color', 'w');
figure('Position',[50,100,400,350])
ax = show(robot,q);
title('Generated robot model');

% Adjust view angle, zoom, and lighting
view(135, 40); % Set custom view angle
camzoom(2.5);  % Zoom in
axis equal;    % Maintain aspect ratio
axis([0 1.5 -0.5 1.0 0.0 2.0])
